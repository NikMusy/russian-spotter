...
PHRASES: list[Phrase] = [
    ...
    Phrase("slow_car_ahead", "медленная машина впереди", P_URGENT, "flags",
           core=True, hard="впереди еле тащится, блядь, объезжай"),
    Phrase("car_stopped_ahead", "машина стоит на трассе", P_URGENT, "flags",
           core=True, hard="машина стоит на трассе, блядь, аккуратно"),
    Phrase("spin_ahead", "впереди развернуло", P_URGENT, "flags", core=True,
           hard="впереди кого-то развернуло нахуй, тормози"),
    Phrase("debris_ahead", "обломки на трассе", P_URGENT, "flags",
           hard="обломки на трассе, блядь, объезжай"),
    Phrase("off_track_ahead", "впереди вылетели с трассы", P_NORMAL, "flags",
           hard="впереди улетели нахуй с трассы"),
    ...
    Phrase("wheel_lost", "мы потеряли колесо, едь в боксы", P_SPOTTER, "damage",
           core=True, hard="блять, мы потеряли колесо, едь в боксы, писька",
           note="кричать - колесо отвалилось, это конец заезда"),
    Phrase("wheel_hanging", "колесо висит, аккуратно в боксы", P_URGENT,
           "damage", hard="колесо на соплях, ползи в боксы"),
    Phrase("rival_wheel_off", "у соперника впереди выпало колесо", P_URGENT,
           "damage", sims=ENDURANCE,
           note="у машины впереди отвалилось колесо - объезжай обломки"),
    ...
    # Женский род: секунда и минута - "одна" и "две", а не "один" и "два".
    Phrase("num_1f", "одна", P_NORMAL, "numbers", core=True,
           note="для секунд и минут: ОДНА секунда"),
    Phrase("num_2f", "две", P_NORMAL, "numbers", core=True,
           note="для секунд и минут: ДВЕ секунды"),

    # Формы слов. По-русски: 1 секунда, 2 секунды, 5 секунд.
    Phrase("seconds_1", "секунда", P_NORMAL, "numbers", core=True,
           note="одна секунда, двадцать одна секунда"),
    Phrase("seconds_2", "секунды", P_NORMAL, "numbers", core=True,
           note="две секунды, две и пять секунды"),
    Phrase("seconds_5", "секунд", P_NORMAL, "numbers", core=True,
           note="пять секунд, одиннадцать секунд"),
    Phrase("lap_1", "круг", P_NORMAL, "numbers", core=True,
           note="один круг"),
    Phrase("laps_2", "круга", P_NORMAL, "numbers", core=True,
           note="два круга"),
    Phrase("laps_5", "кругов", P_NORMAL, "numbers", core=True,
           note="пять кругов"),
    Phrase("minute_1", "минута", P_NORMAL, "numbers", core=True),
    Phrase("minutes_2", "минуты", P_NORMAL, "numbers", core=True),
    Phrase("minutes_5", "минут", P_NORMAL, "numbers", core=True),
    Phrase("percent_1", "процент", P_NORMAL, "numbers", core=True),
    Phrase("percent_2", "процента", P_NORMAL, "numbers", core=True),
    Phrase("percent_5", "процентов", P_NORMAL, "numbers", core=True),
    Phrase("degree_1", "градус", P_INFO, "numbers", core=True),
    Phrase("degrees_2", "градуса", P_INFO, "numbers", core=True),
    Phrase("degrees_5", "градусов", P_INFO, "numbers", core=True),
    ...
    Phrase("in_slipstream", "ты в слипстриме, держись за ним", P_NORMAL,
           "gaps", core=True, sims=ENDURANCE,
           note="в эндурансе DRS нет, тянет слипстрим"),
    Phrase("in_slipstream_close", "плотно в мешке, готовь обгон", P_NORMAL,
           "gaps", core=True, sims=ENDURANCE,
           hard="сел ему на хвост, дави",
           note="совсем близко, слипстрим тащит сильнее всего"),
    ...
    Phrase("time_left", "до конца осталось", P_NORMAL, "laps", core=True,
           hard="блядь, до конца осталось",
           note="дальше число и слово минут - мат только в начале, "
                "иначе разорвёт склейку"),
    Phrase("hour_to_go", "остался час гонки", P_NORMAL, "laps", core=True,
           hard="остался час, блядь, терпим"),
    Phrase("final_minutes", "последние минуты, дотерпи", P_NORMAL, "laps",
           hard="последние минуты, блядь, дотерпи"),
    ...
    Phrase("lapped_car_ahead", "круговой впереди", P_NORMAL, "endurance",
           core=True, sims=ENDURANCE, hard="круговой впереди, объезжай нахуй"),
    ...
    Phrase("energy_ok", "энергии хватает, атакуй", P_INFO, "energy",
           core=True, sims=(SIM_LMU,), hard="энергии дохуя, атакуй"),
    ...
    Phrase("energy_low", "виртуальная энергия на исходе", P_NORMAL, "energy",
           core=True, sims=(SIM_LMU,), hard="энергия на нуле, блядь"),
    ...
    Phrase("battery_full", "батарея заряжена, полный дэплой", P_INFO,
           "energy", sims=(SIM_LMU,), hard="батарея полная, вваливай нахуй"),
    Phrase("battery_low", "батарея разряжена, дэплоя мало", P_NORMAL,
           "energy", sims=(SIM_LMU,), hard="батарея сдохла нахуй, дэплоя нет"),
    Phrase("lift_and_coast", "работай лифт энд кост, береги топливо",
           P_NORMAL, "energy", sims=ENDURANCE,
           hard="лифт энд кост, не жги топливо нахуй"),
    ...
    # Итог сессии: звучит на финише, дальше подставляется позиция.
    Phrase("quali_result", "квалификация закончена, ты отквалился", P_NORMAL,
           "session", core=True, note="дальше позиция: ...отквалился ТРЕТЬИМ"),
    Phrase("race_result", "финиш, ты приехал", P_NORMAL, "session", core=True,
           hard="финиш, блядь, ты приехал", note="дальше позиция: ...приехал ТРЕТЬИМ"),
    Phrase("started_from", "а стартовал", P_NORMAL, "session", core=True,
           note="дальше позиция: а стартовал ВОСЬМЫМ"),
    Phrase("practice_over", "практика закончена", P_NORMAL, "session",
           core=True),
    Phrase("gained_places", "отыграл мест", P_NORMAL, "session", core=True,
           hard="отыграл мест, красава", note="дальше число"),
    Phrase("lost_places", "потерял мест", P_NORMAL, "session", core=True,
           hard="потерял мест, блядь", note="дальше число"),
    Phrase("held_position", "приехал как стартовал", P_NORMAL, "session",
           core=True),

    # Творительный падеж: "приехал третьим", "стартовал восьмым".
    Phrase("as_1", "первым", P_NORMAL, "session", core=True),
    Phrase("as_2", "вторым", P_NORMAL, "session", core=True),
    Phrase("as_3", "третьим", P_NORMAL, "session", core=True),
    Phrase("as_4", "четвёртым", P_NORMAL, "session", core=True),
    Phrase("as_5", "пятым", P_NORMAL, "session", core=True),
    Phrase("as_6", "шестым", P_NORMAL, "session", core=True),
    Phrase("as_7", "седьмым", P_NORMAL, "session", core=True),
    Phrase("as_8", "восьмым", P_NORMAL, "session", core=True),
    Phrase("as_9", "девятым", P_NORMAL, "session", core=True),
    Phrase("as_10", "десятым", P_NORMAL, "session", core=True),
    Phrase("as_11", "одиннадцатым", P_NORMAL, "session"),
    Phrase("as_12", "двенадцатым", P_NORMAL, "session"),
    Phrase("as_13", "тринадцатым", P_NORMAL, "session"),
    Phrase("as_14", "четырнадцатым", P_NORMAL, "session"),
    Phrase("as_15", "пятнадцатым", P_NORMAL, "session"),
    Phrase("as_16", "шестнадцатым", P_NORMAL, "session"),
    Phrase("as_17", "семнадцатым", P_NORMAL, "session"),
    Phrase("as_18", "восемнадцатым", P_NORMAL, "session"),
    Phrase("as_19", "девятнадцатым", P_NORMAL, "session"),
    Phrase("as_20", "двадцатым", P_NORMAL, "session"),
